...
export default function InvoiceCardModalTitle({ cardData }) {
    ...
    const options = [
        {
            ...
            label: (
                <span
                    style={{ fontSize: "12px" }}
                    className={`badge unpaid-status m-0 fw-bold`}
                >
                    {" "}
                    UNPAID
                </span>
        ...
        },
        {
            ...
            label: (
                <span
                    style={{ fontSize: "12px" }}
                    className={`badge paid-status m-0 fw-bold`}
                >
                    {" "}
                    PAID
                </span>
        ...
        setTimeout(async () => {
            if (
                ...
                try {
                    ...
                    message =
                        err?.data?.message ||
                        `${err?.status} code ${err?.originalStatus}` ||
                        "An error occurred";
                    variant = "error";
                } finally {
    ...
    useEffect(() => {
        ...
        setPaydate(
            cardData.invoice_paid_date
                ? dayjs(cardData.invoice_paid_date).format("YYYY-MM-DD")
                : cardData.invoice_paid_date
        );
    }, [cardData]);
    ...
    return (
        <>
            <div className="d-flex flex-wrap align-items-center m-0 p-1">
                ...
                <span className="h6 fw-semibold ms-auto m-0">
                    Invoice Date:
                </span>
                <span className="h6 fw-bold ms-2 text-end m-0 me-4">
                    {" "}
                    {dayjs(cardData.invoice_date).format("MMM DD, YYYY")}
                </span>
            </div>
            <div className="d-flex flex-wrap align-items-center m-0 p-1 mb-2">
                ...
                {IsEdit ? (
                    ...
                    <span
                        style={{ fontSize: "14px" }}
                        className={`badge ${
                            Selected == "PAID" ? "paid-status" : "unpaid-status"
                        } m-0 fw-bold`}
                    >
                        {" "}
                        {Selected}
